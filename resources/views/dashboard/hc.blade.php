    ...
    <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-50">
        ...
        >
            ...
            <nav class="px-4 py-6 space-y-2">
                @php
                    $navItems = [
                        ['label' => 'Data Karyawan', 'route' => 'karyawans.*', 'icon' => 'fas fa-user', 'href' => route('karyawans.index')],
                        ...
                        ['label' => 'Users', 'route' => 'users.*', 'icon' => 'fas fa-users', 'href' => route('users.index')],
                    ];
                @endphp
        ...
        <main class="flex-1 p-8 overflow-y-auto mt-16">
            ...
            <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                @php
                $cards = [
                    ['label' => 'Total Karyawan', 'value' => $totalKaryawan, 'icon' => 'fas fa-users', 'color' => 'bg-blue-600', 'hover' => 'bg-blue-700'],
                    ['label' => 'KPI Indikator Aktif', 'value' => $totalIndikatorAktif, 'icon' => 'fas fa-bolt', 'color' => 'bg-yellow-400', 'hover' => 'bg-yellow-500'],
                    ['label' => 'Kategori KPI', 'value' => $totalKategoriKpi, 'icon' => 'fas fa-th-list', 'color' => 'bg-green-500', 'hover' => 'bg-green-600'],
...
<br>
    ...
    function drillDown(tahun, semester, label) {
        fetch(`/chart-detail/${tahun}/${semester}`)
            .then(res => res.json())
            .then(data => {
